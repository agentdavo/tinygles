...
namespace fp
{
    ...
    void gl_print_matrix(const tGLfixed *m)
    {
        for (int i = 0; i < 4; i++)
        {
            fprintf(stderr, "%f %f %f %f\n",
                    ...
                    static_cast<double>(m[4 + i]),
                    static_cast<double>(m[8 + i]),
                    static_cast<double>(m[12 + i]));
        }
    }
    ...
    void gl_print_matrix2(const tGLfixed *m)
    {
        for (int i = 0; i < 4; i++)
        {
            std::cout << static_cast<double>(m[i]) << "  "
                      << static_cast<double>(m[4 + i]) << " "
                      << static_cast<double>(m[8 + i]) << " "
                      << static_cast<double>(m[12 + i]) << std::endl;
        }
    }
    ...
    void glScalex(tGLfixed x, tGLfixed y, tGLfixed z)
    {
        ...
        m[0] = fp::multiply(m[0], x);
        m[1] = fp::multiply(m[1], y);
        m[2] = fp::multiply(m[2], z);
        m[4] = fp::multiply(m[4], x);
        m[5] = fp::multiply(m[5], y);
        m[6] = fp::multiply(m[6], z);
        m[8] = fp::multiply(m[8], x);
        m[9] = fp::multiply(m[9], y);
        m[10] = fp::multiply(m[10], z);
        m[12] = fp::multiply(m[12], x);
        m[13] = fp::multiply(m[13], y);
        m[14] = fp::multiply(m[14], z);
        gl_matrix_update(c);
    }
    ...
    void glOrthox(tGLfixed left, tGLfixed right, tGLfixed bottom, tGLfixed top, tGLfixed near, tGLfixed far)
    {
        ...
        // Constants as tGLfixed
        tGLfixed two = tGLfixed(2);
        tGLfixed one = tGLfixed(1);
        tGLfixed zero = tGLfixed(0);
        ...
        // Multiply the current matrix by the orthographic projection matrix
